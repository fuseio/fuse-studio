      ...
      if (walletModule === 'CommunityManager') {
        try {
          ...
          if (isFunderDeprecated) {
            ...
            const funderJob = await taskManager.now('fundToken', { phoneNumber, receiverAddress: walletAddress, identifier, tokenAddress, communityAddress, bonusType: 'join' }, { isWalletJob: true, isFundTokenJob: true })
