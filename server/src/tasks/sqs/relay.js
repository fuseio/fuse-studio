      ...
      if (walletModule === 'CommunityManager') {
        try {
          ...
          if (isFunderDeprecated) {
            ...
            const funderJob = await taskManager.now('fundToken', { phoneNumber, accountAddress: walletAddress, identifier, tokenAddress, originNetwork, communityAddress, bonusType: 'join' }, { isWalletJob: true })
