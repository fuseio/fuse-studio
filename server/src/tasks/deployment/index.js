...
const deploy = withAccount(async (account, { communityProgressId }) => {
  try {
    ...
    if (steps.bridge) {
      ...
      const { name, communityURI, plugins, description, adminAddress } = steps.community.args
      await new Community({
        plugins,
        communityAddress,
        isClosed,
      ...
      const { homeTokenAddress, name, communityURI, plugins, description, adminAddress } = steps.community.args
      await new Community({
        plugins,
        communityAddress,
        isClosed,
