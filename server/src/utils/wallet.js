...
const createTransactions = (_tx) => {
  const tx = addressessToLowerCase(_tx)
  const { watchedAddress } = tx

  if (tx.source === 'wallet') {
    return [tx]
  }
  if (tx.asset === 'ETH' && !tx.netBalanceChanges && !tx.contractCall && !tx.tokenAddress) {
...
const processTransaction = async (_tx) => {
  console.log(`receiving tx ${_tx.hash} for address ${_tx.watchedAddress} with status ${_tx.status} from ${_tx.source}`)
  console.log(_tx)
  if (_tx.status === 'speedup') {
    console.log(`ignoring the tx ${_tx.hash} because of the status ${_tx.status}`)
    return
  }
  const txs = createTransactions(_tx)
